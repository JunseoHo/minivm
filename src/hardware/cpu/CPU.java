...
import hardware.intr.HIRQ;
import hardware.intr.HWName;
import hardware.intr.HW_ISR;
import os.SIRQ;
import os.SWName;
...
public class CPU extends Component<HIRQ> implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            ...
            decode();
            execute();
            MiniVMUtils.sleep(150); // down clock
        }
    }

    private void registerISR(int intrId, HW_ISR isr) {
        if (intrId < 0 || isr == null) return;
    ...
    private void INT() {
        queue.add(new HIRQ(HWName.CPU, (int) IR_OPERAND_L, IR_OPERAND_R));
    }
    ...
    private class Timer implements Runnable {
        ...
        public void run() {
            while (true) {
                if (++time > 2) queue.add(new HIRQ(HWName.MEMORY, HIRQ.TIME_SLICE_EXPIRED));
